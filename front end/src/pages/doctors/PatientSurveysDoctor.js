  ...
  return (
    ...
    >
      ...
      <Modal.Footer>
        ...
        <Button onClick={() => props.onConfirm(treatmentSelected, (dateSelected.toString() + "T" + timeSelected.toString()))} disabled={dateSelected==="" || timeSelected==="" || treatmentSelected==="" || treatmentSelected ==="-1" }>Enviar</Button>
      </Modal.Footer>
    </Modal>
  );
}
...
const PatientSurveysDoctor = () => {
  ...
  const getQueryStatus = () => {
    SurveyService.getSurveysFromPatient(params.id).then(
      (response) => {
...
export default PatientSurveysDoctor;
